    ...
    <section id="github" className="py-24 px-6">
      <div className="max-w-6xl mx-auto">
        <FadeIn>
          <SectionHeading
            title="On GitHub"
            subtitle="Public repos — work in progress and experiments"
          />
        </FadeIn>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {githubRepos.map((repo, i) => (
            <FadeIn key={repo.name} delay={i * 80}>
              <div className="p-6 rounded-xl bg-white/5 border border-white/10 hover:border-teal/50 transition-all duration-300 hover:-translate-y-1">
                <div className="flex items-center gap-2 mb-3">
                  <svg
                    className="w-4 h-4 text-offwhite/40"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={1.5}
